...
console.log(data.data);
  return (
    <div>
      <NextSeo title="Pesanan sedang dikirim" />
        ...
        <div className="content col-span-5 ml-5">
          <ul className="mt-5 flex flex-row w-full">
            <Link href="/member/order/ongoing">
              <li className=" text-gray-400 font-semibold  pb-2  cursor-pointer ">
                Pesanan diproses
              </li>
            </Link>
            <Link href="/member/order/onsent">
              <li className="text-blue-100 font-semibold  border-b-2 pb-2  border-blue-400 ml-5  cursor-pointer">
                Sedang dikirim
              </li>
            </Link>
          ...
          <div className="w-full mt-10">
            {data.data.length  == 0 && <span>Belum Ada Pesanan  Yang sedang dikirim</span>}
...
export async function getServerSideProps({ req }) {
  ...
  const res = await fetch(`${process.env.API_V2}/api/transaction/code/3`, {
    headers: new Headers({
