...
function Complated({ user, data }) {

  const router = useRouter();

  return (
    <div>  
      <NextSeo title="Pesanan Selesai " />
        ...
        <div className="content col-span-5 ml-5">
          <ul className="mt-5 flex flex-row w-full">
            ...
            <Link href="/member/order/completed">
              <li
                className="    text-blue-100 font-semibold border-b-2 pb-2 ml-5 border-blue-100
              cursor-pointer"
              >
                Selesai
              </li>
          ...
          <div className="w-full mt-10">
            {data.data.length == 0 && (
              <span>Belum Ada Pesanan Yang sudah selesai</span>
            )}
...
export async function getServerSideProps({ req }) {
  ...
  const res = await fetch(`${process.env.API_V2}/api/transaction/code/1`, {
    headers: new Headers({
